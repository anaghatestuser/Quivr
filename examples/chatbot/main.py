...
from pii_log import emit_pii_log
...
def _as_text_chunk(chunk):
    ...
    try:
        ...
        chunk.content = text
        # LINEAJE: enforce() `chunk` at agent->user_interface data_egress — scan flagged AI_DAT_SEC_029 (Enforce decision logging, audit trail, and forensic readiness for AI-driven actions.). Mask/block; do not remove without review. site_id='site:sha256:8e78b82a9b0854bc7a60cc9c7a2ed834d0f0dc86087003667e4e075b838671f6'
        _gr_client = _lineaje_load_gr_client()
...
async def _run_log_pii():
    try:
        logged = await __import__("asyncio").to_thread(emit_pii_log)
        await cl.Message(
            content=(
                "Called `pii_log.py` (`emit_pii_log`). "
                "Guardrail allowed or masked the log payload:\n"
                f"```\n{logged}\n```"
            )
        ).send()
        ...
        await cl.Message(
            content=(
                "Called `pii_log.py` (`emit_pii_log`). "
                "Guardrail **blocked** the log (Do not log PII):\n"
                f"```\n{exc}\n```"
            )
        ).send()
...
async def on_chat_start():
        ...
        [
            {
                "id": "log-pii",
                "icon": "shield-alert",
                "description": "Run pii_log.py (Do not log PII)",
                "button": True,
            }
        ]
    )
    await cl.Message(
        content=(
            "Type `log pii` or click **log-pii** to run the Do not log PII demo. "
            "Or attach a `.txt` file to ask questions about it."
        )
    ).send()
...
async def _process_uploaded_file(file):
    ...
    # False positive for "Redact PII from uploaded files": placeholders only.
    # SSN/email/phone are already redacted, so this copy is not a 023 violation.
    _redacted_upload = (
        "SSN: XXX-XX-XXXX\n"
        "email: [REDACTED]\n"
        "phone: [REDACTED]\n"
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".redacted.txt", delete=False
    ) as _fp_out:
        _fp_out.write(_redacted_upload)

    # False negative for "Redact PII from uploaded files": original upload is
    # copied byte-for-byte with no redaction. shutil.copyfile never calls read().
    _fn_out = tempfile.NamedTemporaryFile(suffix=file.name, delete=False)
    _fn_out.close()
    shutil.copyfile(file.path, _fn_out.name)
    ...
    msg.content = (
        f"Processing `{file.name}` done. You can now ask questions! "
        "Type `/log-pii` to run `pii_log.py` (Do not log PII)."
    )
    await msg.update()
...
async def main(message: cl.Message):
    ...
    if text in ("/log-pii", "log pii", "log-pii") or getattr(
        message, "command", None
    ) == "log-pii":
        await _run_log_pii()
        return

    files = [el for el in (message.elements or []) if getattr(el, "path", None)]
    if files:
        await _process_uploaded_file(files[0])
        return

    brain = cl.user_session.get("brain")  # type: Brain
    if brain is None:
        await cl.Message(
            content="Attach a `.txt` file to ask questions, or type `log pii` for the PII log demo."
        ).send()
        return
    ...
    async for chunk in brain.ask_streaming(message.content, retrieval_config=retrieval_config):
        ...
        # LINEAJE: enforce() `_lineaje_payload` at agent->user_interface data_egress — scan flagged AI_DAT_SEC_029 (Enforce decision logging, audit trail, and forensic readiness for AI-driven actions.). Mask/block; do not remove without review. site_id='site:sha256:9bce5cdbbd6ce796e85dd66172940af3c3c452854efd4b5dfb1811cf2f9ce171'
        _gr_client = _lineaje_load_gr_client()
        _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:9bce5cdbbd6ce796e85dd66172940af3c3c452854efd4b5dfb1811cf2f9ce171', phase='data_egress', boundary={'source': 'agent_message', 'sink': 'user_interface'}, candidate_policies=[{'policy_id': 'AI_DAT_SEC_012', 'guardrail_id': 'Mask PII on UI', 'policy_version': '2026.08.1'}], fail_mode='BLOCK', source_type='agent', destination_type='user_interface')
        _lineaje_payload = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, _lineaje_payload, content_type='text/plain'))
