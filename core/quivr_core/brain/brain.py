...
def _lineaje_load_gr_client():
    ...
    try:
        _mod = gr_check(_mod, "agent", "user_interface", site_id='site:sha256:7b97b43cc2156105f1951f2b96dde30a8b4a25c55a6e28162835d701795883df')
    ...
    for file in await storage.get_files():
        try:
            if file.file_extension:
                ...
                _lineaje_payload_93 = f"processing {file} using class {processor_cls.__name__}"
                try:
                    import asyncio as _gr_asyncio
                    _lineaje_payload_93 = await _gr_asyncio.to_thread(gr_check, _lineaje_payload_93, "agent", "log", site_id='site:sha256:f2691307f34833bb0a19cc007a6ae256946125c32bbecde051672c42abd5483f')
                except Exception as _gr_exc:
                    if type(_gr_exc).__name__ == "GRBlockedError":
                        raise
                ...
                # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_VULN_SEC_007 (AI systems must implement incident detection, structured logging, and reporting mechanisms). Mask/block; do not remove without review. site_id='site:sha256:29d7a3e92f2def825dfd36011afa5f09d9b2c0a5cec8e364ded2bd062b644ac8'
    ...
    * Stream the answer to the question.
    ...
    async def afrom_files(
        ...
        *,
        ...
        _lineaje_payload_377 = f"uploaded all files to {storage}"
        try:
            import asyncio as _gr_asyncio
            _lineaje_payload_377 = await _gr_asyncio.to_thread(gr_check, _lineaje_payload_377, "agent", "log", site_id='site:sha256:f2691307f34833bb0a19cc007a6ae256946125c32bbecde051672c42abd5483f')
        except Exception as _gr_exc:
            if type(_gr_exc).__name__ == "GRBlockedError":
                raise
            _lineaje_payload_377 = _lineaje_payload_377
            import logging as _lineaje_logging
            _lineaje_logging.getLogger("lineaje.gr_client").warning(
                "Lineaje guardrail unavailable at 'agent->log' — passing data through unchecked"
            )
        ...
        _lineaje_payload_392 = f"added {len(docs)} chunks to vectordb"
        try:
            import asyncio as _gr_asyncio
            _lineaje_payload_392 = await _gr_asyncio.to_thread(gr_check, _lineaje_payload_392, "agent", "log", site_id='site:sha256:f2691307f34833bb0a19cc007a6ae256946125c32bbecde051672c42abd5483f')
        except Exception as _gr_exc:
            if type(_gr_exc).__name__ == "GRBlockedError":
                raise
            _lineaje_payload_392 = _lineaje_payload_392
            import logging as _lineaje_logging
            _lineaje_logging.getLogger("lineaje.gr_client").warning(
                "Lineaje guardrail unavailable at 'agent->log' — passing data through unchecked"
            )
